-- Nayagement: perbaikan mandiri untuk detail task.
-- Jalankan SELURUH file ini sekali di Supabase SQL Editor, lalu refresh aplikasi.
-- Aman dijalankan ulang: tidak menghapus task, catatan, atau lampiran yang sudah ada.

begin;

-- Pastikan tabel keanggotaan yang dipakai RLS tersedia.
...
-- Owner workspace harus selalu memiliki keanggotaan agar akses RLS tidak tertolak.
...
-- Kolom detail pada task.
alter table public.project_tasks
  add column if not exists progress_percentage smallint,
  add column if not exists brief text;

update public.project_tasks
set progress_percentage = case when status::text = 'completed' then 100 else 0 end
...
-- Hak akses eksplisit untuk tabel yang ditambahkan setelah skema utama.
...
-- Verifikasi cepat: hasil harus menunjukkan 2 kolom dan 2 tabel tersedia.
select
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'project_tasks' and column_name = 'brief'
  ) as brief_ready,
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'project_tasks' and column_name = 'progress_percentage'
  ) as progress_ready,
  to_regclass('public.project_task_notes') is not null as notes_ready,
  to_regclass('public.project_task_attachments') is not null as attachments_ready;
