...
import { ArrowUpRight, ChevronDown, CircleDollarSign, Filter, Plus, Search, UsersRound } from 'lucide-react'
...
export function ClientsPage({ clients, onOpenClient, onCreateClient }: ClientsPageProps) {
  ...
  return (
    <div className="module-page clients-page">
      ...
      <section className="clients-list-card">
        <div className="clients-list-header"><span>Klien</span><span>Status</span><span>Proyek</span><span>Revenue</span><span>Order terakhir</span><span /></div>
        <div className="clients-list-body">{filtered.map((client) => <button key={client.id} className="client-list-row" onClick={() => onOpenClient(client)}><span className="client-identity"><Avatar initials={client.initials} variant={client.accent} /><span><strong>{client.company}</strong><small>{client.name}</small></span></span><span className={`client-status client-status-${client.status.toLowerCase()}`}>{client.status}</span><span>{client.projects} proyek</span><strong>{rupiah(client.revenue, true)}</strong><span>{client.lastOrder}</span><ArrowUpRight size={17} /></button>)}</div>
